module grd2hull
  ...
  !< @details
  !! Defines a data-type for edge segment of a hull.
  type ej

      !< @note modification needed for 3D.
     ! (1=x;2=y , 1=pt1;2=pt2)
     real*8, dimension(2,2) :: x  !< a 2D array of point coordinates (x or y , pt1 or pt2)
     integer :: bc, neigh

  end type ej

  !< @details
  !! Defines a data-type for a hull.
  type hull

     logical :: is_active, is_bn  !<
     type(ej), dimension(:), allocatable :: ejs !< An array for the edges of a hul
...
contains
  ...
  subroutine eq_poly(xc, r, n, hl)
    ...
    real*8, intent(in) :: r
    integer, intent(in) :: n !< Number of the sides of the hull
    type(hull) :: hl
  ...
  subroutine push_bottom_hull(hls, hl)
    ...
    ! local vars
    integer :: n
    type(hull), dimension(:), allocatable :: tmp !< @todo Could we write it as type(hulls) :: tmp too?
