...
contains
  ...
  !> @details Adds more points over the element depending on the value of
  !! eltypein argument \n
  ...
  subroutine add_more_points(grd, pin, eltypein, tolerance, galtype, n1, n2)
    implicit none
    type(grid), intent(inout) :: grd
    integer, dimension(:), intent(in) :: eltypein
    integer, dimension(:), intent(in) :: pin
    real*8, intent(in) :: tolerance
    character(len = 2), intent(in), optional :: galtype !< Type of Galerkin method = PG, DG
    integer, dimension(:), optional :: n1
    integer, dimension(:), optional :: n2
    ...
    if ( grd%nquad4 .eq. 0 ) then ! do it the old way
      ...
      tmp = -1
      ! Sets the first three numbers of each row equal to the icon
      ! i.e.: the first 3 values of the tmp array corresponding to
      ! each element (row) are the cell vertices.
      tmp(:,1:3) = grd%icon
    else ! mixed element
      npemax = (pmax + 1)**2
      ! resize grd%icon
      ...
      select case ( grd%elname(j) )
        ...
        case (GEN_QUADRI)
          ...
          if( present(n1) .and. present(n2) ) then
            tn1 = n1(j); tn2 = n2(j)
            ! check if n1 and n2 are consistent ...
          ...
          ! transforming to the physical element coordinates
          ! using exact transformation
          do k = 1, grd%npe(j)
  ...
  !> @details Converts order \$f p \$f to the number of required points;
  !! npe : number of points per element = \$f p+1 \$f
  !! nped : number of points per edge = \$f (p+1)(p+2)/2 \$f
  subroutine p2node(p, npe, nped)
    implicit none
    integer, intent(in)  :: p      !< Order of the element.
    integer, intent(out) :: npe    !< # of points per element.
    integer, intent(out) :: nped   !< # of points per edge.
  ...
  !> @details Finds points per edge of the element
  ...
  !> @details
  !! Transforms a general master element r (or xi) : 0->1, s (or eta) : 0->1
  !! to physical coordinate
  !! for linear and curved (nonlinear) elements. \n
  !! Input: grid, elem (element number), xi, eta , tolerance
  !! Output: x and y (physical coordinates)
  ...
  !> @details Maps the point coordinates of points on a
  !! quad4 master element to the physical coordinates.
  !! ================================= \n
  !! ============  MAP =============== \n
  !! ================================= \n
  !!                <edge : y3> \n
  ...
  subroutine transform_elem_quadri(grd, elem, xi, eta, x, y, tol)
    ...
    if ( twist .eq. 1 ) then ! twisted!
      ...
      if (twist .eq. 1) then ! still twisted wow !!!
        print *, 'the quad element #', elem, 'is twisted and cannot be fixed! stop'
        stop
      end if
