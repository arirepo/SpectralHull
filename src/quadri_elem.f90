...
contains
  ...
  subroutine pt_dist_1d(n, xmin, xmax, method, x)
    ...
    real*8, intent(in) :: xmin  !< Min position in 1D (input).
    real*8, intent(in) :: xmax	!< Max position in 1D (input).
    character(len = *), intent(in) :: method	!< Method of interest to generate points: 'cheby' or 'equal_space' (input).
    ...
    select case (method)
      ...
      case default
        ...
        print *, 'Cannot recognize the method for 1d point distribution. stop!!'
        stop
  ...
  subroutine fill_elem_coords(x1, x2, x, y)
    ...
    real*8, dimension(:), allocatable :: y  !< Output array in the 2nd dimension.
    ...
    if (allocated(x)) then
      if (size(x) > 0) then
        print *, 'Cannot fill quad4 element because <x> might have' &
    ...
    if (allocated(y)) then
      if (size(y) > 0) then
        print *, 'Cannot fill quad4 element because <y> might have' &
  ...
  subroutine gen_master_quadri(n1, n2, xmin, xmax, ymin, ymax, method, x, y)
    ...
    character(len = *), intent(in) :: method !< Method of interest to generate points: 'cheby' or 'equal_space'.
