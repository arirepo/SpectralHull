module spline
  ...
  public :: spline_nurbs_alloc
  public :: spline_nurbs_comp
  public :: spline_nurbs_eval
  public :: spline_nurbs_eval2
! public :: spline_eval
...
contains
  ...
  subroutine spline_nurbs_alloc(cmx, cmy, ca, cb, cc, cd, cp, dp, np, bt)
  ...
  end subroutine spline_nurbs_alloc

  subroutine spline_nurbs_comp(x, y, a, b, c, d, cp, dp, Mx, My, t, mode, bt)
  ...
  end subroutine spline_nurbs_comp

  subroutine spline_nurbs_eval2(val, x, y, a, b, c, Mx, My, t, xv, yv, opt, bt)
    ...
    if(btype == spline_i)then
      ...
      call nurbs_eval(Mx, My, np, inv, xv, yv, a, b, opt)
    end if
  end subroutine spline_nurbs_eval2

  subroutine spline_nurbs_eval(x, y, t, tt, xx, yy, opt, mode, bt)
    real(rk), dimension(:), intent(in)     :: x, y, t, tt
    real(rk), dimension(:), intent(in out) :: xx, yy
    character(len=*),       intent(in)     :: opt, mode
    integer, optional,      intent(in)     :: bt
    real(rk), dimension(:), allocatable :: mx, my, a, b, c, d, ct, cp, dp
    integer :: np, i
    real(rk) :: val

    if(present(bt) .and. bt == nurbs_i)then
      np = size(x)
      allocate(ct(np))
      call spline_nurbs_alloc(mx, my, a, b, c, d, cp, dp, np, bt)
      do i = 1, size(tt)
        val = (tt(i) - tt(1)) / (tt(size(tt)) - tt(1))
        call nurbs_eval(mx, my, np, val, xx(i), yy(i), a, b, opt)
      end do
      deallocate(mx, my, a, b, c, d, ct)
    else
      call spline_eval(t, x, tt, xx, opt, mode)
      call spline_eval(t, y, tt, yy, opt, mode)
    end if
  end subroutine spline_nurbs_eval
